...
from utils.non_della_dist_utils import setup, cleanup
from utils.utils import get_lr
from tqdm import tqdm
from benchmarks.hellaswag import iterate_examples, render_example, get_most_likely_row

# To run: torchrun -m --nproc_per_node=8 pretrain.non_della_pretrain --model {model}
def main():
    # Distributed setup
    ddp = int(os.environ.get('RANK', -1)) != -1
    device, local_rank, rank, world_size, master_process = setup(ddp)
    ...
    for step in pbar:
            ...
            if master_process:
                ...
                if step > 0 and (step % (5000 // scale) == 0 or last_step):
                    if val_loss_accum.item() < best_val_loss:
                        ...
                        print(f'Validation loss improved at step {step}! Saving the model to {checkpoint_path}.')
        ...
        lr = get_lr(step, warmup_iters, max_steps, max_lr, min_lr)
